...
photo = PhotoImage(file = './img/CupDog.png')
...
print(len(photoAry))    # 360,000 개 리스트가 생성
...
# 색상들을 정리. 중간값보다 작으면 검은색, 127보다 크면 흰색
...
for i in range(h):
    for k in range(w):
        r=g=b=photoAry[pos]
        pos += 1
        photo.put(f'#{r:02x}{g:02x}{b:02x}', (i,k)) # photo에 각 위치의 색상을 photoAry에 들어잇는 값으로 변경
